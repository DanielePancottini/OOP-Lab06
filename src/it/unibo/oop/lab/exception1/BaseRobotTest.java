    ...
    public void testRobotMovementBase() {
        ...
        for (int i = 0; i < RobotEnvironment.WORLD_X_UPPER_LIMIT; i++) {
            ...
            try {
            	...
            	assertNotNull("[CHECKING EXCEPTION MESSAGE]", e.getMessage());
            	fail("Battery or Position incoherent: " + e.getMessage());
            };
        }
        // reached the right limit of the world
        try {
        	//r1.moveRight();
        	fail("World X Upper Limit Reached, cannot move");
        ...
        try {
        	//r1.moveUp();
        	fail("World Y Upper Limit Reached, cannot move");
        } catch (PositionOutOfBoundException e) {
        	assertNotNull("[CHECKING EXCEPTION MESSAGE]", e.getMessage());
        }
